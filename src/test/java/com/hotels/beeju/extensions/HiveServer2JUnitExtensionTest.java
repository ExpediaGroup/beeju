...
public class HiveServer2JUnitExtensionTest {
  ...
  public void showCreateTable() throws Exception {
    ...
    String expectedShowCreateTable = new StringBuilder()
        ...
        .append("LOCATION\n")
        .append("  'file:" + server.getTempDirectory() + "/" + DATABASE + "/my_table'\n")
  ...
  public void createDatabase() throws Exception {
    ...
    assertThat(db.getLocationUri(), is(String.format("file:%s/%s", server.getTempDirectory(), databaseName)));
  }
  ...
  public void addPartition() throws Exception {
    ...
    try {
      ...
      assertThat(partitions.get(0).getSd().getLocation(),
          is(String.format("file:%s/%s/%s/partcol=1", server.getTempDirectory(), DATABASE, tableName)));
    } finally {
      client.close();
    }
  }
  ...
  public void dropPartition() throws Exception {
    ...
    try {
      ...
      partition.getSd().setLocation(
          String.format("file:%s/%s/%s/partcol=1", server.getTempDirectory(), DATABASE, tableName));
